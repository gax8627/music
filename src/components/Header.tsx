...
export const Header = ({
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-20 px-4 sm:px-8 md:px-12 py-3.5 sm:py-6 flex items-center justify-between">
      {/* Left: RG Music Logo (non-navigable in private view) */}
      {isPrivateView ? (
        <div className="text-base sm:text-lg md:text-xl tracking-tight text-white font-bold flex items-center gap-2 select-none cursor-default">
          {logoContent}
        </div>
      ) : (
        <a
          href="#"
          className="text-base sm:text-lg md:text-xl tracking-tight text-white font-bold flex items-center gap-2 select-none hover:opacity-90 transition-opacity"
        >
          {logoContent}
        </a>
      )}
      ...
      <div className="flex items-center gap-2 liquid-glass rounded-full px-3 sm:px-4 py-1 sm:py-1.5 text-[11px] sm:text-xs text-white/90 select-none shadow-lg">
        ...
        <span className="font-mono tracking-wide font-medium">
          {isPrivateView ? 'Studio Live · Private Audition' : `${totalSongs} Songs · ${totalTime}`}
        </span>
      </div>
    </header>
  );
